...
import { lineString, booleanPointOnLine, booleanPointInPolygon, point, polygon, polygonToLine } from '@turf/turf';

import { getTranslation } from '../helpers';

Draw.PolygonSplitter = Draw.extend({
  initialize(map) {
    this._map = map;
    this._shape = 'Line';
    this.toolbarButtonName = 'drawPolygonSplitter';
  ...
  _createVertex(e) {
    ...
    // var p = point([1, 1]);
    // var l = lineString([[-1, -1],[1, 1],[1.5, 2.2]]);
    // console.log(p);
    // console.log(l);
    // console.log(booleanPointOnLine(p, l));

    // console.log('--');


    //console.log(this._layer._latlngs.length);
    ...
    this._map.pm.findLayers().forEach(layer => {
      //let line = polygon(layer.toGeoJSON().geometry.coordinates);
      let line = polygonToLine(layer.toGeoJSON());
      //console.log(layer._leaflet_id, line);
      let isOnLine = booleanPointOnLine(pt, line);
      //let isOnLine = booleanPointInPolygon(pt, line);
      if (isOnLine) {
        console.log(layer._leaflet_id, isOnLine);
      }
    });
